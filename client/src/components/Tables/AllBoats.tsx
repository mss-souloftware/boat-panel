...
const AllBoats: React.FC<CategoryComponentProps> = ({ category }) => {

  const [boats, setBoats] = useState<Boats[]>([]);
  const [boatsCat, setBoatsCat] = useState<Boats[]>([]);
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="px-4 py-6 md:px-6 xl:px-7.5">
        <h4 className="text-xl font-semibold text-black dark:text-white">
          {category ? (
            <span>Boats For Category {category}</span>
          ) : (
            <span>All Boats</span>
          )
          }
      ...
      {boatsCat.map((boat, key) => (
        ...
        >
          ...
          <div className="col-span-1 flex items-center">
            <Link href={`/allboats/${boat.id}`} className="hover:text-primary">
            ...
            <button onClick={() => handleDelete(boat.id)} className="hover:text-primary ml-2">
